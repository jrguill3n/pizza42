...
import { ChevronRight, Sparkles, ShoppingBag, Repeat, CheckCircle, Zap, Clock } from "lucide-react";
import { Button } from "@/components/ui/button";
import { Header } from "@/components/layout/header";
import { BottomNav } from "@/components/layout/bottom-nav";
import { MenuItemCard } from "@/components/menu/menu-item-card";
import { useAuth, useCart } from "@/components/providers/app-provider";
import { featuredItems, applyLastOrderToCartMock } from "@/lib/mock-data";
...
export default function HomePage() {
  ...
  const hasOrders = user && user.orders_context.orders_count > 0;
  const lastOrderItems = user?.orders_context.last_order_items;

  // Calculate last order total
  const lastOrderTotal = lastOrderItems?.reduce(
    (sum, item) => sum + item.price * item.quantity, 0
  ) ?? 0;

  const handleRepeatOrder = () => {
    const items = applyLastOrderToCartMock(lastOrderItems);
    items.forEach((item) => {
      addItem(item);
    });
    ...
    <div className="min-h-screen bg-background pb-24 md:pb-0">
      ...
      <main className="container mx-auto px-4 md:px-6 py-6 md:py-10 max-w-4xl">
          ...
          <div className="relative z-10">
            ...
            <div className="flex items-center gap-3 mb-5">
              <div className="w-14 h-14 rounded-2xl bg-primary/15 flex items-center justify-center border border-primary/20">
                <span className="text-2xl font-black text-primary neon-text-readable">42</span>
              </div>
              <div>
        ...
        <section className="mb-6">
          {isAuthenticated && hasOrders && lastOrderItems ? (
            <div className="glass-elevated rounded-2xl overflow-hidden">
              ...
              <div className="px-5 py-4 border-b border-border/20 bg-accent/5">
                <div className="flex items-center gap-2.5">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">Ready for another order?</p>
                  </div>
                </div>
              </div>
              ...
              <div className="px-5 py-4">
                ...
                <div className="flex items-center justify-between py-3 border-t border-border/20">
                  ...
                  <span className="text-lg font-bold text-primary">${lastOrderTotal.toFixed(2)}</span>
                </div>
              ...
              <div className="flex items-start gap-4">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-sm text-muted-foreground leading-relaxed mb-4">
                    Build your perfect pizza in under 30 seconds. We{"'"}ll remember your favorites.
                  </p>
                  <Button
                    asChild
        ...
        <section>
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-lg font-bold text-foreground tracking-tight">Featured</h2>
            <Link
              href="/order"
